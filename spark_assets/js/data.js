/* Spark v2 — Data Layer (localStorage) */
const DB = {
  // ── IDEAS ──
  ...
  addIdea(idea){
    ...
    idea.stage = idea.stage || 'raw';
    idea.author = idea.author || this.getUser() || 'Anonymous';
    idea.timeline = [{ stage:'raw', note:'Idea captured', author: idea.author, ts: idea.createdAt }];
    list.unshift(idea);
    this.saveIdeas(list);
    return idea;
  },
  ...
  addTimelineEntry(ideaId, stage, note){
    ...
    if(!idea) return;
    const entry = { stage, note: note||'', author: this.getUser()||'Anonymous', ts: Date.now() };
  ...
  addBoardIdea(idea){
    ...
    idea.createdAt = Date.now();
    idea.author = this.getUser() || 'Anonymous';
    idea.votes = [];
    idea.claims = [];
    list.unshift(idea);
    this.saveBoard(list);
  ...
  toggleVote(boardId){
    const user = this.getUser()||'Anonymous';
  ...
  addBoardComment(boardId, text){
    ...
    const c = { id:'bc_'+Date.now(), boardId, author: this.getUser()||'Anonymous', content:text, createdAt:Date.now() };
    all.push(c);
  ...
  setUser(name){ localStorage.setItem('spark_user', name) },
  ...
  clearAll(){
    ['spark_ideas','spark_projects','spark_comments','spark_reactions','spark_board','spark_board_cmts'].forEach(k=>localStorage.removeItem(k));
  }
};
